...
// ===================================================
void quickSort(vector<int>& arr, float left, int right) {
...
int main() {
    ...
    if (mode == 1)
        ...
        verifyFunc = [](const vector<int>& arr) { return isSortedSample(arr, 20); };
    else if (mode == 3)
        verifyFunc = isSortedOddIndex;
    else if (mode == 4)
        verifyFunc = isSortedEvenIndex;
    else if (mode == 5)
        verifyFunc = [](const vector<int>& arr) { return isSortedHeadTailSample(arr, 5); };
    else if (mode == 6)
        verifyFunc = [](const vector<int>& arr) { return isSortedMiddleSample(arr, 5); };
    else {
    ...
    auto totalTime =
        chrono::duration_cast<chrono::nanoseconds>(globalEnd - globalStart).count();

    cout << "\nTotal Execution Time: " << totalTime << " ns\n";
